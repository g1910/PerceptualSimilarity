...
class DistModel(BaseModel):
    ...
    def initialize(self, model='net-lin', net='alex', pnet_rand=False,
        ...
        if (self.model == 'net-lin'):  # pretrained net + linear layer
            self.net = networks.PNetLin(use_gpu=use_gpu, pnet_rand=pnet_rand,
                                        pnet_tune=pnet_tune, pnet_type=net,
                                        use_dropout=True, spatial=spatial,
                                        version=version, gpu_ids=gpu_ids)
            kw = {}
            if not use_gpu:
                kw['map_location'] = 'cpu'
            ...
            if (not is_train):
                ...
                # strict false for scaling and vgg parameters
        ...
        # if use_gpu:
        #     self.net.to_gpu(use_gpu, gpu_ids)
    ...
    def forward(self, in0, in1, retNumpy=True):
        ...
        def convert_output(d0):
            if (retNumpy):
                ...
                if not self.spatial:
                    ...
                    return ans[0, ...].transpose([1, 2,
                                                  0])  # Reshape to usual numpy image format: (height, width, channels)
                return ans
            else:
                return d0
        ...
        if self.spatial:
            ...
            L = [skimage.transform.resize(x, spatial_shape,
                                          order=self.spatial_order, mode='edge')
                 for x in L]
